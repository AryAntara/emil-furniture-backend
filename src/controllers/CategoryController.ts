...
export class CategoryController extends BaseController {
    ...
    async list(c: Context) {
        ...
        const response = await this.categoryService.findWithPage(page, limit, orderColumn, orderType) as any;
        response.queries = queries;
        return c.json(this.respond(response, true, 'Mendapatkan list data kategori.'))
    ...
    async insert(c: Context) {
        ...
        if (!await this.categoryService.insert(data)) return c.json(this.respond(null, false, "Tidak dapat menambahkan kategori."), 500);
    ...
    async update(c: Context) {
        ...
        if (!validation.success && validation.sendError) return validation.sendError(c);

        const { data } = validation;
        delete data.category_id;

        if (!await this.categoryService.updateById(categoryId, data)) return c.json(this.respond(
            null, false, "Gagal memperbarui data kategori"
        ));

        return c.json(this.respond(null, true, 'Berhasil memperbarui data kategori.'))
