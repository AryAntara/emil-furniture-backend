...
export class AddressController extends BaseController {
    ...
    async list(c: Context) {
        ...
        const addressEntries = await this.addressService.findByUserId(userId as number);
        return c.json(this.respond(addressEntries, true,
    ...
    async insert(c: Context) {
        ...
        if (await this.addressService.isLimitReached(userId)) return c.json(this.respond(null, false, `Tidak bisa menambahkan alamat lagi, kamu sudah punya ${AddressService.LIMIT_ADDRESS_COUNT}`), 400)
        if (!await this.addressService.insert(data)) return c.json(this.respond(null, false, "Tidak bisa menambah alamat."), 500)
    ...
    async update(c: Context) {
        ...
        if (!await this.addressService.updateById(addressId, data)) return c.json(this.respond(
            null, false, "Gagal memperbarui data alamat."
        ), 500)

        return c.json(this.respond(
            null, true, "Berhasil memperbarui data alamat."
        ))
    }
    ...
    async delete(c: Context) {
        ...
        if (!await this.addressService.deleteById(addressId)) return c.json(this.respond(
            null, false, "Gagal menghapus data alamat."
        ), 500)
    ...
    async setActive(c: Context) {
        ...
        if (!await this.addressService.activeAddress(addressId))
            return c.json(this.respond(null, false, "Tidak dapat mengaktifkan alamat."), 500)

        return c.json(this.respond(null, true, "Berhasil mengaktifkan alamat."), 500)
    }
}