...
export class AuthMiddleware {
  ...
  async validateAccessToken(c: Context, next: Next) {
    ...
    if (!bearerToken)
      return c.json(respond(null, false, "Token sudah tidak valid lagi"), 401);
    ...
    if (!userId) {
      return c.json(
        respond(
          null,
          false,
          "Token sudah tidak valid lagi, Mohon login ulang."
        ),
        401
      );
    }
  ...
  async validateAdminAccess(c: Context, next: Next) {
    ...
    if (!isAdmin && !isSuperUser) {
      return c.json(respond(null, false, "Akses tidak diizinkan."));
    }
