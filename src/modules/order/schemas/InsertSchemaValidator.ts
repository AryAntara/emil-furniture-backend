...
import { orderDetailService, productService, stockService } from "../../../providers";
import { MUST_A_NUMBER_ERROR, PRODUCT_NOT_FOUND_ERROR, QTY_IS_ZERO_ERROR, STOCK_NOT_ENOUGH_ERROR } from "./constants";


const insertSchema = z.object({
  product_id: z.number({ message: MUST_A_NUMBER_ERROR }),
  qty: z.number({ message: MUST_A_NUMBER_ERROR }).min(1, {message: QTY_IS_ZERO_ERROR}),
});
...
async function validationStockWasEnough(item: {
  ...
  const productId = item.product_id,
    ...
    lockedStock = await orderDetailService.getLockedStock(productId);
