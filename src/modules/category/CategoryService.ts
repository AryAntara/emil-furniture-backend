...
export class CategoryService implements CategoryServiceInterface {
    ...
    async findWithPage(page: number, limit: number, orderColumn: string, orderType: string): Promise<{ categoryEntries: Array<Category>, categoryCount: number }> {
        const offset = (page - 1) * limit,
            order = [[orderColumn, orderType]] as Order,
            categoryEntries = await this.categoryRepository.findWithOffsetAndLimit(offset, limit, order, [
                'id', 'name'
            ]),
