...
export class AddressService implements AddressServiceInterface {
    ...
    async isLimitReached(userId: number): Promise<boolean> {
        return await this.addressRepository.countBy({
            userId
    ...
    async findByUserId(userId: number): Promise<Address[]> {
        return await this.addressRepository.findByUserId(userId, [
            'title', 'description', 'isActived', 'id'
        ]);
    }

    // check address was exist or not by given address id
    async isExistsById(addressId: number, userId?: number): Promise<boolean> {
        if (userId) return await this.addressRepository.isExists({
            id: addressId,
            userId: userId
        });

        return await this.addressRepository.isExistsById(addressId);
    }
    ...
    async updateById(addressId: number, data: any): Promise<boolean> {
        ...
        return await this.addressRepository.updateById(addressId, data);
    }
    ...
    async activeAddress(addressId: number): Promise<boolean> {
        ...
        return await this.addressRepository.updateById(addressId, {
            isActived: "yes"
        });
    }
}